...
ORIENT_HEAD_TOWARD = os.getenv("ORIENT_HEAD_TOWARD", "BOTTOM").upper() # "TOP" or "BOTTOM"
...
def segment_person(img):
    ...
    out = mp_seg.process(rgb)
    if out.segmentation_mask is None:
        return None, 0.0
    mask = out.segmentation_mask
    conf = float(np.mean(mask))
    th = max(0.3, min(0.6, conf + 0.05))
    bin_mask = (mask > th).astype(np.uint8) * 255
...
def yolo_pose_classify(img_bgr):
    ...
    def get(i):
        if i < len(kxy):
            return float(kxy[i,0]), float(kxy[i,1]), float(kcf[i])
    ...
    # Head vs feet using nose vs ankles when available
    feet_y = np.nanmean([ly, ry])
    head_first = True
    if not (math.isnan(ny) or math.isnan(feet_y)):
        head_first = (ny > feet_y) if ORIENT_HEAD_TOWARD=="BOTTOM" else (ny < feet_y)

    # Supine vs prone: if face missing, use shoulders
    shoulders_good = (scL > 0.25 and scR > 0.25)
    supine = (nc >= 0.25) or shoulders_good
    ...
    # Confidence: blend cues
    conf_terms = [c for c in [nc, scL, scR, lc, rc] if c > 0]
    mean_c = float(np.mean(conf_terms)) if conf_terms else 0.0
    conf = float(round(0.42 + 0.25 * min(1.0, mean_c), 3))

    why = f"yolo_fallback nose={nc:.2f} shoulders={(scL+scR)/2:.2f} ankles={(lc+rc)/2:.2f}"
...
def _override_head_first_with_ankles(seg, yolo_res):
    """
    Decide head/feet using ankle y-positions from YOLO.
    Returns (head_first_bool_or_None, used_override_bool).
    Never returns None by itself; the first item can be None if no data.
    """
    try:
        ...
        kxy = r0.keypoints.xy[0].cpu().numpy()
        kcf = None
        if r0.keypoints.conf is not None and len(r0.keypoints.conf) > 0:
            kcf = r0.keypoints.conf[0].cpu().numpy()
        ...
        # Require some ankle evidence
        if ly is None and ry is None:
            return (None, False)
        mean_c = (lc + rc) / 2.0
        if mean_c < 0.35:   # ankle confidence cutoff
            return (None, False)

        # Decide head_first from vertical ordering
        # If your camera orientation expects head toward BOTTOM:
        ORIENT = os.getenv("ORIENT_HEAD_TOWARD", "BOTTOM").upper()
        feet_y = np.nanmean([v for v in [ly, ry] if v is not None])
        # Use seg’s PCA endpoints if available
        head_first = True  # default
        if "head_end" in seg and "feet_end" in seg and seg["head_end"] is not None and seg["feet_end"] is not None:
            he_y = float(seg["head_end"][1]); fe_y = float(seg["feet_end"][1])
            head_first = (he_y > fe_y) if ORIENT == "BOTTOM" else (he_y < fe_y)
        else:
            # fallback purely from ankles vs image center
            H = seg.get("mask").shape[0] if isinstance(seg.get("mask"), np.ndarray) else None
            if H is not None:
                head_first = (H/2.0 > feet_y) if ORIENT == "TOP" else (H/2.0 < feet_y)
...
def decide_hybrid(img, want_debug=False):
    """
    Fuse Segmentation (PCA + Haar/MediaPipe face cue) with YOLOv8-Pose.

    - Uses ankles to override head/feet if face is missing (more stable).
    - Downweights decubitus calls when torso axis is very straight.
    - Blends confidence when both branches agree on a plain posture.
    - Strong supine safeguards:
        A) If prone but face OR nose is detected  -> flip to supine (high conf).
        B) If prone but axis is straight <25° AND (ankles strong OR shoulders ok)
           -> flip to supine (moderate-high conf).

    Returns: best, seg, yolo_res
    """
    # Run both branches
    ...
    # Pull diagnostics from seg
# Prefer top-level cues; fall back to debug if needed
    ...
    is_decub  = _is_decubitus(seg.get("label"))

    # 1) If seg thinks decubitus but body axis is straight, soften that claim
    ...
    # 2) Override head/feet using YOLO ankles (safe unpack)
    res_hf = _override_head_first_with_ankles(seg, yolo_res)
    hf_new, used_override = res_hf if isinstance(res_hf, (tuple, list)) and len(res_hf) == 2 else (None, False)

    if used_override:
        def _flip_headfeet(label: str, pos: str, head_first_now: bool):
        ...
        # apply to best
        b_lab, b_pos = _flip_headfeet(best.get("label",""), best.get("position",""), hf_new)
        if (b_lab, b_pos) != (best.get("label"), best.get("position")):
            best = dict(best)
            best["label"], best["position"] = b_lab, b_pos
            best["why"] = (best.get("why") or "") + " | headfeet=ankle_override"

        # also apply to seg branch for consistent overlays
        s_lab, s_pos = _flip_headfeet(seg.get("label",""), seg.get("position",""), hf_new)
        if (s_lab, s_pos) != (seg.get("label"), seg.get("position")):
            seg = dict(seg)
            seg["label"], seg["position"] = s_lab, s_pos

    # 3) Prefer YOLO if seg says decubitus but YOLO claims a plain posture confidently
    if is_decub and not _is_decubitus(yolo_res.get("label")):
        if yolo_res.get("confidence", 0.0) >= 0.55 and seg.get("confidence", 0.0) <= 0.70:
            best = yolo_res

    # 4) If both are plain and agree, smooth the confidence by blending
    if (not _is_decubitus(seg.get("label"))) and (not _is_decubitus(yolo_res.get("label"))):
        if seg.get("label") == yolo_res.get("label"):
            best = dict(best)
            best["confidence"] = float(min(1.0, 0.5*seg["confidence"] + 0.5*yolo_res["confidence"]))

    # 5) Strong supine safeguards (ensure this is NOT nested under #4)
    nose_c, shoulders_c, mean_ank = _nose_shoulder_confidence(yolo_res)

    if best.get("label") in ("head_first_prone", "feet_first_prone"):
        flipped = False

        # A) Face OR nose seen -> force supine (highest priority)
        ...
        # B) If no face: axis straight + ankles strong (or shoulders ok)
        if not flipped and axis_vert is not None and axis_vert < 25.0 and (mean_ank >= 0.40 or shoulders_c >= 0.25):
            best = dict(best)
            best["label"]      = best["label"].replace("prone", "supine")
            best["position"]   = best["position"].replace("Prone", "Supine")
            best["confidence"] = float(max(best.get("confidence", 0.55), 0.75))
            best["why"]        = (best.get("why") or "") + " | supine_override(straight_axis+ankles/shoulders)"
        print("RESULT:", best["label"], best["confidence"], best["why"], flush=True)
...
async def analyze(image: UploadFile = File(...), debug: int = Query(0)):
    try:
        ...
        out = {"position": best["position"], "label": best["label"], "confidence": float(best["confidence"]), "why": best["why"], **({"debug": dbg} if dbg else {})}
...
async def overlay(image: UploadFile = File(...)):
    try:
        ...
        pad = 12
        cv2.rectangle(vis, (10,10), (10+tw+pad*2, 10+th+pad*2), (0,0,0), -1)
        cv2.putText(vis, label_txt, (10+pad, 10+th+pad), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2, cv2.LINE_AA)
