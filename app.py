...
ORIENT_HEAD_TOWARD = os.getenv("ORIENT_HEAD_TOWARD", "BUTTOM").upper()  # "TOP" or "BOTTOM"
...
def _apply_headfeet_sticky(label: str):
    ...
    If we detect an immediate flip (current != last), reset history first
    so a recent orientation change isn’t dragged back by old votes.
    """
    global _hf_hist
    is_hf = label.startswith("head_first_")

    # If current decision flips vs last, clear old bias
    if len(_hf_hist) > 0 and is_hf != _hf_hist[-1]:
        _hf_hist.clear()
...
def _override_head_first_with_ankles(seg, yolo_res):
    ...
    Changes vs old:
      - Accept override if AT LEAST ONE ankle has conf >= 0.35 (or mean >= 0.35)
      - Prefer segmentation endpoints if present (sign consistent with ORIENT_HEAD_TOWARD)
      - Midline guard to avoid flipping when feet near image center
    """
    try:
        ...
        # NEW: be more permissive—trust one good ankle or decent mean
        ank_ok = (lc >= 0.35) or (rc >= 0.35) or ((lc + rc) / 2.0 >= 0.35)
        if not ank_ok:
            return (None, False)
        ...
        # Prefer segmentation endpoints if available (consistent sign)
        ...
        # Otherwise, compute using image midline as a fallback reference
        H = None
        m = seg.get("mask")
        if isinstance(m, np.ndarray):
            H = m.shape[0]
        if H is None and hasattr(r0, "orig_shape"):
            try:
        ...
        # Don’t override if feet are too close to midline (ambiguous)
...
def decide_hybrid(img, want_debug=False):
    ...
    Applies ankle override, supine safeguards, head/feet enforcement from keypoints,
    and debouncing (sticky) + face-aware environment fallback.
    """
    seg = classify_segmentation(img, want_debug=want_debug)
    yolo_res = yolo_pose_classify(img)

    # Pick the more confident stream as the starting "best"
    ...
    # Penalize unlikely decubitus if the long axis is very vertical
    ...
    # --- Ankle-driven override of head/feet (robust, earlier threshold) ---
    ...
    if used_override and hf_new is not None:
        ...
        # Keep seg internally consistent for visualization
    ...
    # If segmentation says decubitus but YOLO is confident otherwise, trust YOLO
    ...
    # If both streams agree and it's not decubitus, blend confidence
    ...
    # ---- Extract YOLO confidences (nose/shoulders/ankles) for logic below ----
    ...
    # ---- Prone -> Supine safeguard (make stricter on face/nose) ----
    if best.get("label") in ("head_first_prone", "feet_first_prone"):
        flipped = False
        # was nose_c >= 0.20; make stricter to avoid false face triggers
        if face_vis >= 0.50 or nose_c >= 0.50:
            best = dict(best)
    ...
    # ---- Enforce head/feet directly from keypoints when cues exist ----
    # If ankles are ~ok and nose is present (even weakly), compute head_first from ny vs feet_y.
    try:
        ...
        if r0.keypoints is not None and r0.keypoints.xy is not None and len(r0.keypoints.xy) > 0:
            ...
            mean_ank_c = (lc + rc) / 2.0
            if ny is not None and (ly is not None or ry is not None) and (mean_ank_c >= 0.35 or lc >= 0.35 or rc >= 0.35) and nc >= 0.15:
                feet_y = np.nanmean([v for v in [ly, ry] if v is not None])
                head_first_now = (ny > feet_y)  # larger y == lower in image
                b_lab, b_pos = _flip(best.get("label",""), best.get("position",""), head_first_now)
                if (b_lab, b_pos) != (best.get("label"), best.get("position")):
                    best = dict(best)
                    best["label"], best["position"] = b_lab, b_pos
                    best["why"] = (best.get("why") or "") + " | headfeet_from_keypoints(nose_vs_ankles)"
    except Exception:
        pass

    # ---- Sticky (debounce) head/feet to avoid flicker across frames ----
    ...
    # ---- Face-aware environment fallback when confidence is low ----
    LOW_CONF = 0.60
    if best.get("confidence", 0.0) < LOW_CONF:
        face_strong = (face_vis >= 0.50) or (nose_c >= 0.50)
        if (not face_strong) and (mean_ank < 0.20):
            if ORIENT_HEAD_TOWARD == "BOTTOM" and best["label"].startswith("head_first_"):
                best = dict(best)
                best["label"] = best["label"].replace("head_first_", "feet_first_")
                best["position"] = best["position"].replace("Head First", "Feet First")
                best["why"] = (best.get("why") or "") + " | forced_env_fallback(BOTTOM)"
            elif ORIENT_HEAD_TOWARD == "TOP" and best["label"].startswith("feet_first_"):
                best = dict(best)
                best["label"] = best["label"].replace("feet_first_", "head_first_")
                best["position"] = best["position"].replace("Feet First", "Head First")
                best["why"] = (best.get("why") or "") + " | forced_env_fallback(TOP)"
