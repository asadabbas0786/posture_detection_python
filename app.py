...
def yolo_pose_classify(img_bgr):
    ...
    - Head/feet: prefer nose vs ankles; if nose missing, infer from ankles vs image center.
    ...
    if not (math.isnan(ny) or math.isnan(feet_y)):
        # Nose and ankles available
        head_first = (ny > feet_y) if ORIENT_HEAD_TOWARD == "BOTTOM" else (ny < feet_y)
    elif not math.isnan(feet_y):
        # Nose missing → fallback
        if ORIENT_HEAD_TOWARD == "TOP":
            # Feet above midline (smaller y) => Feet First
            head_first = not (feet_y < (H * 0.5))
        else:  # BOTTOM
            # Feet below midline (larger y) => Feet First
            head_first = not (feet_y > (H * 0.5))

    if head_first is None:
        head_first = True  # neutral default
...
def decide_hybrid(img, want_debug=False):
    ...
    - Downweight decubitus when axis is very straight.
    - Override head/feet from ankles when necessary.
    - Strong supine safeguards, even if segmentation failed.

    Returns:
        best: dict with keys {label, position, confidence, why, ...}
        seg:  full segmentation-branch dict (includes mask/head_end/feet_end if any)
        yolo_res: YOLO branch dict (includes raw 'res')
    """
    # ---- run both branches ----
    ...
    # ✅ Always apply ankle override, even if segmentation failed
        best = dict(best)
        best["label"], best["position"] = _flip(best.get("label", ""), best.get("position", ""), hf_new)
        best["why"] = (best.get("why") or "") + " | applied_ankle_override"

        # Also update seg for consistent overlays
        seg = dict(seg)
        seg["label"], seg["position"] = _flip(seg.get("label", ""), seg.get("position", ""), hf_new)
